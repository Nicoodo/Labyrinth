...
{
  int i;
  cout<<"cellvoisins "<<cell->m_nb_neighb<<endl;
  if (cell->m_notsaved)
  {
    cout<<"traitement"<<endl;
    *f << *cell;
    cell->m_notsaved = false;
    cout<<cell->m_x<<endl;
...
{
  ofstream osft(file_name);
  if(!osft.is_open())
    cout<<"Erreur d'ouverture de " <<file_name<<endl;
  else
  {   
    osft << "# Start:"<<endl;
    osft << "("<<maze.c0->m_x<<","<<maze.c0->m_y<<")"<<endl;
    osft << "# End:"<<endl;
    osft << "("<<maze.cf->m_x<<","<<maze.cf->m_y<<")"<<endl;
    osft << "# Cells:"<<endl;
    Cell* cinit = new Cell(maze.c0->m_x,maze.c0->m_y);
    cinit->m_neighb = maze.c0->m_neighb;
    cinit->m_nb_neighb = maze.c0->m_nb_neighb;
    cout<<"co"<<maze.c0->m_nb_neighb<<endl;
    cout<<"cinit "<<cinit->m_nb_neighb<<endl;
    save_cell(cinit,&osft);
  }
  osft.close();
}
void read_maze(const string& file_name)
{
  ifstream rdft(file_name);
  if(!rdft.is_open())
    cout<<"Erreur d'ouverture de"<< file_name<<endl;
  else
  {
